...
testSEMnoiseimg = "../test/SEMnoise"
SEMnoise_Gaussion_img = "../test/result/gassion"
SEMnoise_Clean_img = "../test/result/clean"


for filename in os.listdir(testSEMnoiseimg):
    if filename.endswith(endword):
        imgname = filename.split('.')
        imgname = imgname[0]
        SEMnoiseimg = imread(testSEMnoiseimg + "/" + filename)
        ...
        SEMnoise_Gaussion_img_path = SEMnoise_Gaussion_img + "/" + imgname + ".tif"
        imsave(SEMnoise_Gaussion_img_path, (Gen_Gaussion_img_save).astype('uint8'))

        SEMnoise_Clean_img_path = SEMnoise_Clean_img + "/" + imgname + ".tif"
        imsave(SEMnoise_Clean_img_path, (Gen_Clean_img_save).astype('uint8'))
